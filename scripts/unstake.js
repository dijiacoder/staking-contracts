 ...
 * 
 * 使用方法:
 * npx hardhat run scripts/unstake.js --network sepolia
 */
async function main() {
  // ZeroStake 合约地址
  ...
  try {
    ...
    // 要取消质押的池ID和数量（这里使用ETH池ID 0作为示例）
    const pid = 0; // ETH 池 ID
    const amount = ethers.parseEther("0.001"); // 取消质押的数量
    ...
    console.log("- Min deposit amount:", poolInfo.minDepositAmount.toString(), "wei");
    ...
    if (userInfo.amount < amount) {
      console.log("\\nWarning: Requested unstake amount is greater than staked amount");
      console.log("Setting unstake amount to user's staked amount");
    ...
    // 取消质押交易
    const tx = await zeroStake.connect(deployer).unstake(pid, amount, {
      nonce: nonce,
      gasLimit: 500000,
    });
    ...
    // 查询更新后的用户信息
    const updatedUserInfo = await zeroStake.userInfo(pid, deployer.address);
    console.log("\\nUpdated user info:");
    console.log("- Amount staked:", ethers.formatEther(updatedUserInfo.amount), "tokens");
    ...
    if (msg.includes("insufficient balance")) console.log("\\nTip: Insufficient staked balance to unstake");
    else if (msg.includes("invalid pid")) console.log("\\nTip: Invalid pool ID");
    else if (msg.includes("withdraw paused")) console.log("\\nTip: Withdrawal is currently paused");
